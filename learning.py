...
class Learner():
    ...
    def __init__(self, model, optimizer, train_loader, valid_loader, loss_fn,
                 ...
                 batch_step_scheduler=True, eval_at_start=False, awp=None):
        self.model = model
    ...
    def valid_epoch(self):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(tqdm_loader):
                with torch.no_grad():
                    inputs, targets = self.to_device(
                        inputs), targets.to(self.device)
                    ...
                    tqdm_loader.set_description(
                        'loss: {:.4}'.format(round(curr_loss_avg, 4)))
    ...
    def train_batch(self, batch_inputs, batch_targets, batch_idx):
        # --- 1. First Forward Pass (Standard) ---
        preds, loss = self.get_loss_batch(batch_inputs, batch_targets)
        loss_value = loss.item()  # For logging

        # --- 2. First Backward Pass (Clean Gradients) ---
        # We ALWAYS need this to know which direction to attack
        loss.backward()

        # --- 3. AWP Logic ---
        if self.awp is not None and self.current_epoch >= self.awp.start_epoch:

            # A. Save weights and inject noise (Perturb)
            self.awp.attack()

            # B. Second Forward Pass (Robust)
            # We must re-calculate loss with the noisy weights!
            # We ignore 'preds' here as we only need the loss for gradients
            _, loss_adv = self.get_loss_batch(batch_inputs, batch_targets)

            # C. Second Backward Pass (Robust Gradients)
            loss_adv.backward()

            # D. Restore Original Weights
            # (But keep the robust gradients calculated in step D)
            self.awp.restore()

        # --- 4. Optimizer Step & Scheduler ---
